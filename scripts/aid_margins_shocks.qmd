...
First, we remove the 'regional' recipients.
...
```{r}
target_pop_period <- target_pop_period_cohort |> 
...
- Create a unique identifier with the following format: XXXX_XXXX_XXXX (`donor_code`_`recipient_code`_`year`)

```{r}

target_pop <- target_pop_period |> 
  mutate(
