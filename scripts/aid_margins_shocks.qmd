...
```{r}
data_disaster[!grepl('regional', recipient_name)]
```
...
```{r}
estimand <- estimand[, `:=` (
  commit_xmarg = ifelse(commitment != 0 & !is.na(commitment), 1, 0),
  disb_xmarg = ifelse(disbursement != 0 & !is.na(disbursement), 1, 0)  
)]
...
estimand_mediator <- estimand_mediator |> 
  mutate(across(starts_with("channel"), \(x) ifelse(!is.na(commitment), replace_na(0), x)))
