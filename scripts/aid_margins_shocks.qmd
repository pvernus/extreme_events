...
List all potential provider-recipient dyads included in the OECD CRS datasets. Two options:
...
We only keep countries which have received disbursements reported to the oECD. In other words, we don't include: (high-income) countries considered as bilateral donors in the CRS data set, their overseas collectivities (ex: la Réunion), and China.
...
We create an unique group identifier for each combination of "donor_name" and "recipient_name".
...
# checks
distinct(data_clean, donor_name)
distinct(assignment, donor_name)

distinct(data_clean, recipient_name)
...
Convert binary outcomes variables to factors
