...
estimand_mediator_treatment <- estimand_mediator_treatment |> 
  mutate(
    event_dummy = ifelse(event_n %in% c(0,NA), 0, 1),
    call_dummy = ifelse(call_n %in% c(0,NA), 0, 1),
    appeal_dummy = ifelse(appeal_n %in% c(0,NA), 0, 1),
    declaration_dummy = ifelse(declaration_n %in% c(0,NA), 0, 1)
  )
```
...
# treatment
assignment |> 
  select(event_n:declaration_dummy) |> 
  summary()
```
...
summarize(assignment,
  ...
  .by = c(recipient_name, year, event_dummy)) |> 
  ggplot(aes(x = event_dummy, y = log(commit), colour = event_dummy)) +
  geom_boxplot() +
  theme_minimal() +
...
plot_treat_binary <- distinct(assignment,
         recipient_name, year, event_dummy) |> 
  mutate(event_dummy = as.numeric(levels(event_dummy))[event_dummy]) |> 
  pivot_wider(names_from = 'year', values_from = 'event_dummy') |> 
  rowwise() |> 
...
plot_call_binary <- distinct(assignment,
         recipient_name, year, call_dummy) |> 
  mutate(call_dummy = as.numeric(levels(call_dummy))[call_dummy]) |> 
  pivot_wider(names_from = 'year', values_from = 'call_dummy') |> 
  rowwise() |> 
  mutate(total = sum(c_across(`2002`:`2022`), na.rm = T)) |> 
  ungroup() |> 
  mutate(across(`2002`:`2022`, as.integer)) |> 
